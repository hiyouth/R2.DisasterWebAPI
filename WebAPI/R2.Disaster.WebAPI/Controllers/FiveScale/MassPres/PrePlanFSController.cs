...
namespace R2.Disaster.WebAPI.Controllers.GeoDisaster.MassPres
{
    ...
    /// 防灾预案相关服务
    /// </summary>
    public class PrePlanFSController : PhyRelationEntityController<PrePlanFS>
    {
        private IPrePlanFSService _preplanService;

        /// <summary>
        /// 构造函数
        /// </summary>
        /// <param name="preplanService"></param>
        public PrePlanFSController(IPrePlanFSService preplanService)
            : base(preplanService)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public PrePlanFS Get(int id)
        {
            if (id <= 0)
                throw new Exception("不存在这样的主键编号");
            //return new string[] { "value1", "value2" };
            PrePlanFS plan = this._preplanService.Get(id);
            return plan;
        }
        ...
        /// <returns></returns>
        public IList<PrePlanFS> GetByUId(string uid)
        {
            if (String.IsNullOrEmpty(uid))
                throw new Exception("防灾预案的统一编号不能是“ ”或者Null");
            List<PrePlanFS> plans = this._preplanService.GetByUId(uid).ToList();
            return plans;
        }
        ...
        /// <returns></returns>
        public IList<PrePlanFS> GetByKeyWords(string keyword)
        {
            if (String.IsNullOrEmpty(keyword))
                throw new Exception("防灾预案的统一编号不能是“ ”或者Null");
            List<PrePlanFS> plans = this._preplanService.GetByKeyWord(keyword).ToList();
            return plans;
        }
        ...
        [HttpPost]
        public void Update([FromBody]PrePlanFS preplan)
        {
            if (preplan == null)
        ...
        [HttpPost]
        public void Delete([FromBody]PrePlanFS preplan)
        {
            if (preplan == null)
                throw new ArgumentNullException("preplan");
